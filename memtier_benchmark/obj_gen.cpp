...
{
    assert(iter < static_cast<int>(m_next_key.size()) && iter >= OBJECT_GENERATOR_KEY_IMPORT);
    ...
    } else if(iter==OBJECT_GENERATOR_KEY_IMPORT) {
        unsigned long long idx = random_range(1, m_next_key[0]);
        // printf("%llu\n",idx);
        k = m_next_key[idx];
    } else {
...
// bool imported_keylist::read_keys_binary()
// {
...
//     uint64_t value;
//     while (file.read(reinterpret_cast<char*>(&value), sizeof(uint64_t))) {
//         std::string val = std::to_string(value);
//         std::size_t len = val.length();
//         key* k = (key*) malloc(len + sizeof(key) + 1);
//         assert(k != NULL);
//         k->key_len = len;
//         memcpy(k->key_data, val.c_str(), len);
//         m_keys.push_back(k);
//     }
...
bool import_object_generator::read_keys(const char* filename)
{
    ...
    //m_next_key.push_back(0);
    uint64_t value;
    while (file.read(reinterpret_cast<char*>(&value), sizeof(uint64_t))) {
        m_next_key.push_back(value);
    }
