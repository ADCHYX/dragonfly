...
void shard_connection::send_set_command(struct timeval* sent_time, const char *key, int key_len,
                                        const char *value, int value_len, int expiry, unsigned int offset, bool other_index) {
    int cmd_size = 0;
    ...
    {
        benchmark_debug_log("server %s: SET(ZADD myset) key=[%.*s] value_len=%u expiry=%u\n",get_readable_id(), key_len, key, value_len, expiry);
        cmd_size = m_protocol->write_command_set_index(key, key_len, value, value_len, expiry, offset);
    }
...
void shard_connection::send_get_command(struct timeval* sent_time,
                                        const char *key, int key_len, unsigned int offset, bool other_index) {
    int cmd_size = 0;
    ...
    {
        benchmark_debug_log("server %s: GET(ZSCORE myset) key=[%.*s]\n", get_readable_id(), key_len, key);
        cmd_size = m_protocol->write_command_get_index(key, key_len, offset);
    }
