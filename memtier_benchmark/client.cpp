...
bool client::create_set_request(struct timeval& timestamp, unsigned int conn_id) {
    ...
    if (res == available_for_conn) {
        if(m_config->data_import)
        {
            m_connections[conn_id]->send_set_command(&timestamp, m_key_buffer, m_key_len, m_key_buffer, m_key_len, m_obj_gen->get_expiry(), m_config->data_offset, m_config->other_index);
            // printf("%s,%u\n",m_key_buffer,m_key_len);
        }
        else
        {
            unsigned int value_len;
            const char *value = m_obj_gen->get_value(key_index, &value_len);
            m_connections[conn_id]->send_set_command(&timestamp, m_key_buffer, m_key_len, value, value_len, m_obj_gen->get_expiry(),m_config->data_offset, m_config->other_index);
        }
    }
...
bool client::create_get_request(struct timeval& timestamp, unsigned int conn_id) {
    ...
    if (res == available_for_conn) {
        m_connections[conn_id]->send_get_command(&timestamp, m_key_buffer, m_key_len, m_config->data_offset, m_config->other_index);
    }
