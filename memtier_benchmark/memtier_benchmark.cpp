...
void usage() {
    fprintf(stdout, "Usage: memtier_benchmark [options]\n"
            ...
            "\n"
            "Extend Function:\n"
            "      --other-index              Use other index to organize data, like hset or zset\n"
            "\n"
            );
...
int main(int argc, char *argv[])
{
    ...
    if (!cfg.data_import) {
        ...
        // }
            obj_gen = new import_object_generator(cfg.data_import, keylist, cfg.no_expiry);
            dynamic_cast<import_object_generator*>(obj_gen)->read_keys(cfg.data_import);
        }
    }
