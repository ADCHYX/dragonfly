...
public:
    ...
    class iterator {
    public:
        typedef typename LevelBin::key_type key_type;
        typedef iterator self;

    private:
        typename LevelBin::child_bin* curbin;
    ...
    public:
        ...
        inline iterator(typename LevelBin::child_bin *c, unsigned short s)
            : curbin(c), curslot(s)
        {}
...
public:
    //*** methods
    explicit inline LevelBin(const allocator_type &alloc = allocator_type()) 
...
public:
    ...
    result_t find(const key_type &key, data_type &value) 
    {
        /*if(is_retraining) {
...
public:
    ...
    inline result_t insert(const key_type& key, const data_type& data)
    {   
...
private:
    ...
    result_t insert_start(const key_type& key, const data_type& value) 
    {
        if(is_retraining){
        ...
        if(m_root) {
            ...
            if(root->children[slot]->slotuse < childslotmax)
                return insert_child(root->children[slot], key, value, &newkey, &newchild);
            ...
            if(slot > 0 && root->children[slot-1]->slotuse < rootslotmax) {
                ...
                result_t r = insert_child(root->children[slot], key, value, &newkey, &newchild);         
                if(newchild){
                    root->lock();
            ...
            result_t r = insert_child(child, key, value, &newkey, &newchild);
            // create m_root
            if (newchild)
            {
    ...
    result_t insert_child(bin *b, const key_type& key, const data_type& value, 
        ...
        {
            //split
            split_child_bin(child, newkey, newchild);
    ...
    void split_child_bin(child_bin* child, key_type* _newkey, bin** _newchild)
    {
        assert(child->isfull());
