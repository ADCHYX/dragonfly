...
template <class key_t, class val_t>
inline bool Masstree<key_t, val_t>::find(const key_t &key, val_t &val)
{
  uint64_t leaf_ver;
  leaf_t *leaf_ptr = locate_leaf(key, leaf_ver);
  int slot = leaf_ptr->find_first_larger_than_or_equal_to(key);
  slot = slot<leaf_ptr->key_n? slot:leaf_ptr->key_n-1;
  return leaf_ptr->vals[slot].read_ignoring_ptr(val);
}
