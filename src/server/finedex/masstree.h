...
class Masstree {
  ...
  class Leaf : public Node {
   public:
    using Node::is_leaf;      // init the node is leaf
    using Node::key_n;        // init ==0
    using Node::keys;
    using Node::locked;
    using Node::parent;
    using Node::version;
  ...
  struct DataSource {
    DataSource(key_t begin, Masstree *buffer);
 ...
 private:
  ...
  node_t *root = nullptr;                            // init root = begin
  leaf_t *begin = nullptr;                           // allocate_leaf()
  std::atomic<uint32_t> size_est;                    // init 0
  std::mutex alloc_mut;                              // used for allocate nodes
  std::vector<uint8_t *> allocated_blocks;           // init reserve(1), contains all the NODEs
  size_t next_node_i = 0;                            // init 0
  //static const size_t node_n_per_block = alt_buf_fanout + 1;    // init 16+1, the index of next node
  //static const size_t node_size = std::max(sizeof(leaf_t), sizeof(internal_t));  //used for alloct block
   static const size_t node_n_per_block;
